...
This creates a problem though, what if a nodeX is in the fringe waiting, and when we expand a nodes' children we find the nodeX? The problem is that we update the 
...
will update the parent equal to the current_node which was expanded. The solution:
...
def ucs(graph, weight, start, end):
    ...
    while True:
        ...
        if(current_node == end): #then we reached goal 
            return ucs_w, backtrace(parent, start, end)
